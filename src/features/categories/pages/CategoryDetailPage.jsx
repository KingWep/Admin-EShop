  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      try {
        ...
        const fetchedCategory = Array.isArray(payloadData) ? payloadData[0] : payloadData;
  ...
  return (
    <PageContainer>
      <div className="flex flex-col justify-between gap-4 py-4 sm:flex-row sm:items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Category Details</h2>
          <div className="mt-1 flex items-center gap-2 text-xs font-medium text-slate-500">
            ...
            <span className="text-slate-300">›</span>
            <span className="text-slate-400">{category?.name || 'Details'}</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={() => navigate('/dashboard/categories')} className="bg-white border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm flex items-center gap-2">
            <HiOutlineArrowLeft className="h-4 w-4" />
            Back to Categories
          </Button>
          <Button onClick={() => navigate(`/dashboard/categories/edit/${id}`)} className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-base font-semibold text-slate-900 mb-6">Category Information</h3>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <p className="text-sm font-medium text-slate-500">Category Name</p>
                  <p className="mt-1 text-base font-semibold text-slate-900">{category.name}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-slate-500">ID</p>
                  <p className="mt-1 text-base font-mono text-slate-900">
                    {category.id}
                  </p>
                </div>
                <div>
                  ...
                  <div className="mt-1">
                    <Badge variant={getStatusBadgeVariant(category.status)}>
                      {category.status === true || category.status === 'active' ? 'Active' : category.status === false || category.status === 'inactive' ? 'Inactive' : 'Active'}
                    </Badge>
                  </div>
                </div>
                <div>
                  ...
                  <p className="mt-1 text-base text-slate-900">
                    {category.created_at ? new Date(category.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium text-slate-500">Updated At</p>
                  <p className="mt-1 text-base text-slate-900">
                    {category.updated_at ? new Date(category.updated_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                  </p>
                </div>
                <div className="sm:col-span-2">
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col items-center">
              <h3 className="text-base font-semibold text-slate-900 self-start w-full mb-6">Category Icon</h3>
              ...
              {category.icon ? (
                ...
                <div className="relative flex items-center justify-center rounded-xl border border-slate-200 bg-slate-50 p-4 w-full aspect-square max-h-64">
                  <span className="text-slate-400 font-medium">No Icon Available</span>
                </div>
              )}
            </div>
          </div>
